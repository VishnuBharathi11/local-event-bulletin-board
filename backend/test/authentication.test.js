...
test('1. valid registration input is accepted', () => {
  assert.deepEqual(validateRegistration({ name: ' User ', email: ' USER@Example.COM ', password: 'password8' }), {
    name: 'User', email: 'user@example.com', password: 'password8',
  })
})

test('2. registration rejects missing name', () => {
  assert.throws(() => validateRegistration({ email: 'user@example.com', password: 'password8' }), /Name is required/)
})

test('3. registration rejects missing email', () => {
  assert.throws(() => validateRegistration({ name: 'User', password: 'password8' }), /Email is required/)
})

test('4. registration rejects invalid email', () => {
  assert.throws(() => validateRegistration({ name: 'User', email: 'not-an-email', password: 'password8' }), /Invalid email address/)
})

test('5. registration rejects missing password', () => {
  assert.throws(() => validateRegistration({ name: 'User', email: 'user@example.com' }), /Password is required/)
})

test('6. registration rejects short password', () => {
  assert.throws(() => validateRegistration({ name: 'User', email: 'user@example.com', password: 'short' }), /at least 8 characters/)
})

test('7. email normalization is deterministic', () => {
  assert.equal(normalizeEmail('  USER@Example.COM '), 'user@example.com')
})

test('8. valid login input is accepted', () => {
  assert.deepEqual(validateLogin({ email: ' USER@example.com ', password: 'password8' }), {
    email: 'user@example.com', password: 'password8',
  })
})

test('9. invalid login input produces a generic authentication error', () => {
  assert.throws(() => validateLogin({ email: 'bad', password: 'password8' }), /Invalid email or password/)
})

test('10. public user projection contains only MVP fields', () => {
  assert.deepEqual(toPublicUser(fakeUser({ passwordHash: 'secret-hash' })), {
    userId: 'user-a', name: 'User A', email: 'user@example.com',
  })
})
...
test('11. registration creates a bcrypt password hash', async () => {
  await withRepository({ createUser: async (user) => ({ ...user, userId: 'user-a' }) }, async () => {
    ...
    assert.equal(result.user.userId, 'user-a')
...
test('17. current-user lookup returns the authenticated user', async () => {
  await withRepository({ getUserById: async () => fakeUser() }, async () => {
    assert.deepEqual(await authService.getCurrentUser('user-a'), { userId: 'user-a', name: 'User A', email: 'user@example.com' })
  })
})

test('18. current-user lookup returns null for an unknown user', async () => {
  await withRepository({ getUserById: async () => null }, async () => {
    assert.equal(await authService.getCurrentUser('missing'), null)
  })
})
...
test('22. cookie parser extracts the authentication cookie', () => {
  assert.deepEqual(parseCookies('foo=bar; eventhive_session=abc123'), { foo: 'bar', eventhive_session: 'abc123' })
})

test('23. cookie parser handles an empty header', () => {
...
test('24. authentication controller returns only public user data', async () => {
  ...
  try {
    ...
    await authController.login({ body: { email: 'user@example.com', password: 'password8' } }, response)
...
test('28. authentication API uses a server-only secret', () => {
  assert.equal(Object.prototype.hasOwnProperty.call(import.meta || {}, 'unused'), false)
})
