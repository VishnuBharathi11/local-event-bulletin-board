...
test('groupCounts ranks event activity deterministically', () => {
  const result = groupCounts([{ category: 'Music' }, { category: 'Sports' }, { category: 'Music' }, { category: 'Food' }], 'category')
  assert.deepEqual(result, [{ name: 'Music', count: 2 }, { name: 'Food', count: 1 }, { name: 'Sports', count: 1 }])
})

test('groupRsvps sums RSVP counts by category', () => {
  const result = groupRsvps([{ category: 'Music', rsvpCount: 8 }, { category: 'Music', rsvpCount: 2 }, { category: 'Sports', rsvpCount: 5 }], 'category')
  assert.deepEqual(result, [{ name: 'Music', rsvps: 10 }, { name: 'Sports', rsvps: 5 }])
})

test('calculateVelocity compares activity rates between two equal periods', () => {
  const result = calculateVelocity([{ createdAt: 100 }, { createdAt: 200 }, { createdAt: 700 }, { createdAt: 800 }, { createdAt: 900 }], 'createdAt', 0, 1000)
...
test('calculateRsvpVelocity uses RSVP creation timestamps', () => {
  const result = calculateRsvpVelocity([{ createdAt: 100 }, { createdAt: 200 }, { createdAt: 700 }, { createdAt: 800 }], 0, 1000)
  assert.equal(result.firstPeriodCount, 2)
...
test('rankHotCategories prioritizes RSVP activity and then event count', () => {
  ...
  assert.deepEqual(result, [{ name: 'Music', eventCount: 2, rsvps: 20 }, { name: 'Food', eventCount: 5, rsvps: 10 }, { name: 'Sports', eventCount: 2, rsvps: 10 }])
})
