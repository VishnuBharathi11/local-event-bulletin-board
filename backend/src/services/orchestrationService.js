...
const INTENTS = Object.freeze({ EVENT_DISCOVERY: 'event_discovery', EVENT_DETAILS: 'event_details', COMMUNITY_DEMAND: 'community_demand', TREND_ANALYSIS: 'trend_analysis', SEMANTIC_EVENT_DISCOVERY: 'semantic_event_discovery', SIMILAR_EVENT_DISCOVERY: 'similar_event_discovery', SEMANTIC_TREND_ANALYSIS: 'semantic_trend_analysis', SEMANTIC_CONFLICT_ANALYSIS: 'semantic_conflict_analysis', UNSUPPORTED: 'unsupported' })
...
function resolveEffectiveFilters(message, history = [], intent = classifyIntent(message)) {
  ...
  if (intent !== INTENTS.EVENT_DISCOVERY) return cleanFilters(current)
...
function buildDeterministicEventResponse(result, filters) { if (!Array.isArray(result) || result.length === 0) { const location = filters.city ? ` in ${filters.city}` : ''; const category = filters.category ? `${filters.category} ` : ''; return `I couldn't find any upcoming ${category}events${location} on EventHive.`.replace(/  +/g, ' ') } const location = filters.city ? ` in ${filters.city}` : ''; const category = filters.category ? `${filters.category} ` : ''; const lines = result.slice(0, 10).map((event) => { const place = event.location || event.neighborhood || event.city; return place ? `• ${event.title} at ${place}.` : `• ${event.title}.` }); return `Here ${result.length === 1 ? 'is' : 'are'} the upcoming ${category}events${location}:\n${lines.join('\n')}`.replace(/  +/g, ' ') }
async function generateGroundedResponse(message, history, intent, toolData, filters) { if (toolData.needsClarification) return 'Please provide the event name or event ID so I can identify the exact event.'; if (toolData.tool === 'getEventDetails' && !toolData.result) return 'I could not find that event in EventHive. Please provide the exact event name or event ID.'; const ai = geminiService.getClient(); const response = await ai.models.generateContent({ model: geminiService.DEFAULT_MODEL, contents: buildGroundedContext({ message, history, intent, toolData }), config: { temperature: 0.2, maxOutputTokens: 700 } }); const text = response.text?.trim(); if (!text) throw Object.assign(new Error('Gemini returned an empty assistant response'), { code: 'GEMINI_EMPTY_RESPONSE' }); if (hasEventEvidence(toolData) && /\b(no|none|couldn['’]?t find|do not have|does not contain|not contain)\b.*\bevent/i.test(text)) return buildDeterministicEventResponse(sanitizeEvidenceForResponse(toolData.result, intent, message), filters); return text }
async function orchestrate({ message, history = [], conversationId }) { const normalized = conversationContext.sanitizeMessage(message); const safeHistory = normalizeHistory(history); const id = typeof conversationId === 'string' && conversationId.trim() ? conversationId.trim().slice(0, 100) : conversationContext.createConversationId(); const intent = classifyIntent(normalized); const filters = resolveEffectiveFilters(normalized, safeHistory, intent); if (intent === INTENTS.UNSUPPORTED) return buildResponseEnvelope({ conversationId: id, intent, grounded: false, clarification: true, tool: null, filters, contextUsed: safeHistory.length, response: 'I can help with EventHive events, event details, community demand, local event trends, semantic event discovery, similar events, and semantic trend analysis.' }); const toolData = await executeTool(intent, filters, normalized); if (!toolData) throw new Error('No tool is available for the detected intent'); const response = await generateGroundedResponse(normalized, safeHistory, intent, toolData, filters); const clarification = Boolean(toolData.needsClarification || (toolData.tool === 'getEventDetails' && !toolData.result)); return buildResponseEnvelope({ conversationId: id, intent, grounded: Boolean(toolData.tool), clarification, tool: toolData.tool, toolArguments: toolData.arguments, filters, contextUsed: safeHistory.length, response }) }
