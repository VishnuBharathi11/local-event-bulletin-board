...
function classifyIntent(message) {
  ...
  if (/\b(semantic trend|semantic trends|trend cluster|trend clusters|emerging themes|semantic clusters)\b/.test(text)) return INTENTS.SEMANTIC_TREND_ANALYSIS
  if (/\b(similar event|similar events|events like this|related events|find similar|similar to)\b/.test(text)) return INTENTS.SIMILAR_EVENT_DISCOVERY
  if (/\b(meaning|about|related to|relevant to|like|similar|conceptually|semantic)\b/.test(text) && /\b(find|show|search|discover|event|events)\b/.test(text)) return INTENTS.SEMANTIC_EVENT_DISCOVERY
...
function isSupportedCategory(value) { return EVENT_CATEGORIES.some((category) => category.toLowerCase() === value.trim().toLowerCase()) }
...
function resolveEffectiveFilters(message, history = [], intent = classifyIntent(message)) {
  ...
  if (intent !== INTENTS.EVENT_DISCOVERY && intent !== INTENTS.SEMANTIC_EVENT_DISCOVERY) return cleanFilters(current)
...
function sanitizeEvidenceForResponse(result, intent, message) {
  if ((!Array.isArray(result) && !result?.clusters) || (intent !== INTENTS.EVENT_DISCOVERY && intent !== INTENTS.SEMANTIC_EVENT_DISCOVERY) || isRsvpQuestion(message)) return result
  if (Array.isArray(result)) return result.map(({ rsvpCount, ...event }) => event)
  return result
}
...
async function executeTool(intent, filters, message) {
  switch (intent) {
    ...
    case INTENTS.SEMANTIC_TREND_ANALYSIS: return { tool: 'semanticTrendAnalysis', arguments: {}, result: await chatbotService.semanticTrendAnalysis({}) }
    ...
    case INTENTS.SEMANTIC_EVENT_DISCOVERY: { const args = cleanFilters({ category: filters.category, city: filters.city, limit: 10 }); return { tool: 'semanticEventSearch', arguments: args, result: await chatbotService.semanticEventSearch(message, args) } }
    case INTENTS.SIMILAR_EVENT_DISCOVERY: { const eventId = extractEventId(message); if (!eventId) return { tool: null, arguments: {}, result: null, needsClarification: true }; const event = await chatbotService.getEventDetails(eventId); if (!event) return { tool: 'similarEventsForEvent', arguments: { eventId }, result: null, eventNotFound: true }; return { tool: 'similarEventsForEvent', arguments: { eventId, limit: 5 }, result: await chatbotService.similarEventsForEvent(event, { limit: 5 }) } }
...
async function generateGroundedResponse(message, history, intent, toolData, filters) { if (toolData.needsClarification) return 'Please provide the event name or event ID so I can identify the exact event.'; if (toolData.eventNotFound) return 'I could not find that event in EventHive. Please provide a valid event ID.'; const ai = geminiService.getClient(); const response = await ai.models.generateContent({ model: geminiService.DEFAULT_MODEL, contents: buildGroundedContext({ message, history, intent, toolData }), config: { temperature: 0.2, maxOutputTokens: 700 } }); const text = response.text?.trim(); if (!text) throw Object.assign(new Error('Gemini returned an empty assistant response'), { code: 'GEMINI_EMPTY_RESPONSE' }); if (hasEventEvidence(toolData) && /\b(no|none|couldn['’]?t find|do not have|does not contain|not contain)\b.*\bevent/i.test(text)) return buildDeterministicEventResponse(toolData.result, filters); return text }
async function orchestrate({ message, history = [], conversationId }) { const normalized = conversationContext.sanitizeMessage(message); const safeHistory = normalizeHistory(history); const id = typeof conversationId === 'string' && conversationId.trim() ? conversationId.trim().slice(0, 100) : conversationContext.createConversationId(); const intent = classifyIntent(normalized); const filters = resolveEffectiveFilters(normalized, safeHistory, intent); if (intent === INTENTS.UNSUPPORTED) return buildResponseEnvelope({ conversationId: id, intent, grounded: false, clarification: true, tool: null, filters, contextUsed: safeHistory.length, response: 'I can help with EventHive events, semantic event discovery, similar events, community demand, and local trends. Ask me to find events, compare related events, or explain what is trending.' }); const toolData = await executeTool(intent, filters, normalized); if (!toolData) throw new Error('No tool is available for the detected intent'); const response = await generateGroundedResponse(normalized, safeHistory, intent, toolData, filters); const clarification = Boolean(toolData.needsClarification || toolData.eventNotFound); return buildResponseEnvelope({ conversationId: id, intent, grounded: Boolean(toolData.tool), clarification, tool: toolData.tool, toolArguments: toolData.arguments, filters, contextUsed: safeHistory.length, response }) }
module.exports = { INTENTS, MAX_HISTORY, MAX_CONTEXT_CHARS, RESPONSE_VERSION, normalizeHistory, classifyIntent, extractCurrentFilters, extractContextFilters, extractFilters, resolveEffectiveFilters, resolveDateRange, extractEventId, isRsvpQuestion, sanitizeEvidenceForResponse, buildGroundedContext, buildResponseEnvelope, executeTool, orchestrate }
