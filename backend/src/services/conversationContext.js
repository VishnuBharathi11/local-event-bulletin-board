...
function sanitizeHistory(history) {
  ...
  const normalized = history.filter((item) => item && (item.role === 'user' || item.role === 'assistant') && typeof item.content === 'string').map((item) => ({ role: item.role, content: item.content.trim().slice(0, MAX_HISTORY_ITEM_CHARS) })).filter((item) => item.content).slice(-MAX_HISTORY_TURNS)
  let total = 0
  const bounded = []
  for (const item of normalized.reverse()) { if (total + item.content.length > MAX_TOTAL_CONTEXT_CHARS) break; bounded.unshift(item); total += item.content.length }
  return bounded
}
...
function rememberConversationContext(conversationId, metadata = {}) {
  ...
  const safeResults = Array.isArray(metadata.resultMetadata) ? metadata.resultMetadata.slice(0, MAX_RESULT_CONTEXT).map((item) => ({ eventId: typeof item.eventId === 'string' ? item.eventId.slice(0, 100) : null, title: typeof item.title === 'string' ? item.title.slice(0, 200) : null, category: typeof item.category === 'string' ? item.category.slice(0, 100) : null, city: typeof item.city === 'string' ? item.city.slice(0, 100) : null })) : []
  conversationStore.set(conversationId, { intent: metadata.intent || null, tool: metadata.tool || null, eventId: typeof metadata.eventId === 'string' ? metadata.eventId.slice(0, 100) : null, eventTitle: typeof metadata.eventTitle === 'string' ? metadata.eventTitle.slice(0, 200) : null, category: typeof metadata.category === 'string' ? metadata.category.slice(0, 100) : null, city: typeof metadata.city === 'string' ? metadata.city.slice(0, 100) : null, query: typeof metadata.query === 'string' ? metadata.query.slice(0, MAX_MESSAGE_CHARS) : null, resultMetadata: safeResults, assistantMetadata: metadata.assistantMetadata && typeof metadata.assistantMetadata === 'object' ? { grounded: Boolean(metadata.assistantMetadata.grounded), clarification: Boolean(metadata.assistantMetadata.clarification) } : null })
}
