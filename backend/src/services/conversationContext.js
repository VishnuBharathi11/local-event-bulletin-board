...
function rememberConversationContext(conversationId, metadata = {}) {
  ...
  if (conversationStore.size >= MAX_CONVERSATIONS && !conversationStore.has(conversationId)) {
    const oldest = conversationStore.keys().next().value
    if (oldest) conversationStore.delete(oldest)
  }
  const safeResults = Array.isArray(metadata.resultMetadata)
    ? metadata.resultMetadata.slice(0, MAX_RESULT_CONTEXT).map((item) => ({
  ...
  conversationStore.set(conversationId, {
    ...
    assistantMetadata: metadata.assistantMetadata && typeof metadata.assistantMetadata === 'object'
      ? { grounded: Boolean(metadata.assistantMetadata.grounded), clarification: Boolean(metadata.assistantMetadata.clarification) }
      : null,
  })
}
...
function getConversationContext(conversationId) {
  ...
  return conversationStore.get(conversationId.trim()) || null
}

function clearConversationContext(conversationId) {
  if (typeof conversationId === 'string' && conversationId.trim()) conversationStore.delete(conversationId.trim())
}

function resetConversationContextStore() {
