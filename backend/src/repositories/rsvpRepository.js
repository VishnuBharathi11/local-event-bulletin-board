...
async function getRecentRSVPs(startTime, endTime = Date.now()) {
  const snapshot = await getRSVPCollection()
  ...
  return snapshot.docs
    ...
    .filter(
      (rsvp) =>
        Number(rsvp.createdAt) >= startTime &&
        Number(rsvp.createdAt) <= endTime,
    )
}

async function hasUserRSVPd(eventId, userId) {
  const firestore = getFirestore()
  const rsvpRef = getRSVPCollection().doc(
    getRSVPId(eventId, userId),
  )
...
async function rsvpToEvent(eventId, userId) {
  ...
  await firestore.runTransaction(async (transaction) => {
    ...
    const startTime = Number(
      eventSnapshot.get('startTime') || 0,
    )

    const endTime = Number(
      eventSnapshot.get('endTime') ||
      eventSnapshot.get('expireAt') ||
      0,
    )
    ...
    if (endTime > 0 && now >= endTime) {
      throw Object.assign(
        new Error('This event has ended.'),
        { code: 'EVENT_ENDED' },
      )
    }
    ...
    if (startTime > 0 && now >= startTime) {
      throw Object.assign(
        new Error(
          'RSVP is closed because this event is ongoing.',
        ),
        { code: 'EVENT_ONGOING' },
      )
    }
