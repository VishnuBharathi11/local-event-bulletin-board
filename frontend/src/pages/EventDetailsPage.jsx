    ...
    <article className="event-details">
      ...
      <div className="event-details__container">
        <div className="event-details__left">
          ...
          <div className="event-details__organizer" aria-label={`Organizer ${event.organizerName || 'Event Organizer'}`}><span>Organizer</span><strong>{event.organizerName || 'Event Organizer'}</strong></div>
          <div className="event-details__facts-left"><div className="event-detail-fact"><svg className="event-detail-fact__icon" aria-hidden="true" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"></rect><line x1="16" y1="2" x2="16" y2="6"></line><line x1="8" y1="2" x2="8" y2="6"></line><line x1="3" y1="10" x2="21" y2="10"></line></svg><div><span className="event-detail-fact__label">Date & time</span><strong>{formatDate(event.startTime)}</strong><span>{formatEventTimeRange(event.startTime, event.endTime)}</span></div></div></div>
          <div className="event-details__rsvp-actions">
            {authenticated ? currentUser?.userId !== event.organizerId ? <><EventActions event={event} going={rsvp.going} isBusy={rsvp.isBusy || rsvp.status === 'loading'} disabled={isOngoing || isExpired} onGoing={handleGoing} onNotGoing={handleNotGoing} />{rsvp.status === 'loading' && <p className="action-message" role="status">Checking your RSVP status…</p>}{rsvp.status === 'error' && <p className="action-message action-message--error" role="alert">{rsvp.error}</p>}</> : <div className="event-details__actions-organizer"><p className="action-message">You are organizing this event. Manage it from your profile.</p></div> : <div className="event-details__login-prompt"><div><strong>Want to attend?</strong><span>Sign in to RSVP to this event.</span></div><Link className="primary-button" to="/login">Login</Link></div>}
          </div>
        </div>
        <div className="event-details__right">
          <div className="event-details__right-panel">
            ...
            <div className="event-details__attendance-section"><p className="event-details__section-eyebrow">Attendance</p><div className="event-details__rsvp" aria-label="RSVP count"><div><strong>{rsvpCount} {rsvpCount === 1 ? 'person is' : 'people are'} going</strong></div>{rsvp.going && <span className="event-details__rsvp-state">You're on the list!</span>}{authenticated && currentUser?.userId === event.organizerId && <span className="event-details__rsvp-state" style={{ color: 'var(--brand)' }}>You are the organizer</span>}</div></div>
            <hr className="event-details__divider" />
            <div className="event-details__location-section"><p className="event-details__section-eyebrow">Location</p>{event.latitude && event.longitude ? <div className="event-details__map-container"><EventMap events={[event]} center={[event.latitude, event.longitude]} zoom={15} height="250px" /></div> : <div className="event-details__map-placeholder"><span>Map not available for this location</span></div>}<div className="event-details__venue-card"><Navigation className="event-details__venue-icon" aria-hidden="true" size={20} /><div className="event-details__venue-info"><strong>{event.location}</strong><span>{[event.neighborhood, event.city].filter(Boolean).filter(v => !isPincode(v)).join(', ')}</span></div></div>{event.latitude && event.longitude && <a href={`https://www.google.com/maps/dir/?api=1&destination=${event.latitude},${event.longitude}`} target="_blank" rel="noopener noreferrer" className="secondary-button event-details__directions-btn" style={{ display: 'inline-flex', alignItems: 'center', gap: '8px' }}><Navigation size={16} /> Get Directions</a>}</div>
          </div>
        </div>
      </div>
    </article>
