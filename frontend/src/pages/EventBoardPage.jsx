...
export default function EventBoardPage() {
  ...
  return (
    <section className="event-page">
      <header
        className={`event-board-header event-board-header--${categorySlug}`}
        style={{ '--category-hero-image': `url("${categoryInfo.image}")` }}
      >
      ...
      {status === 'success' && (
        <>
          ...
          <div className="map-toggle-container">
            <button className={`map-toggle-button ${viewMode === 'list' ? 'map-toggle-button--active' : ''}`} onClick={() => setViewMode('list')}>List View</button>
            <button className={`map-toggle-button ${viewMode === 'map' ? 'map-toggle-button--active' : ''}`} onClick={() => setViewMode('map')}>Map View</button>
          </div>
          ...
          {rawEvents.length === 0 ? (
            ...
            <div className="event-map-view">
              ...
              {discovery.districtEvents.some(e => !e.latitude) && (
                <p style={{ marginTop: '12px', fontSize: '13px', color: 'var(--text-muted)' }}>* Some events don't have map locations yet and are not shown on the map.</p>
              )}
            </div>
          ) : (
